...
```{python}
continuous = ["age", "trestbps","chol", "thalach","oldpeak"] # specify continuous vars
# z score
z = StandardScaler() # create empty z-score object
...
Then, we'll z-score our continuous inputs. Only `age`, `testbps`, `chol`, `thalach` and `oldpeak` are continuous, so we'll z score those. Because we're going to do hyperparameter tuning to choose `k` (the number of neighbors to look at), we'll create a z-score object using `make_column_transformer`. This function takes in two *very* important arguments. 
...
Then, we'll z-score our continuous inputs. Only `age`, `testbps`, `chol`, `thalach` and `oldpeak` are continuous, so we'll z score those.
...
#### Build and Fit the Model
Now, we're ready to build the model. We'll use only our continuous predictors with a `GaussianNB` model, however we could use the categorical predictors with a `CategoricalNB` model as well (you can even multiply the output of the two models to get a single Naive Bayes model that uses both! See [here](https://stackoverflow.com/questions/14254203/mixing-categorial-and-continuous-data-in-naive-bayes-classifier-using-scikit-lea)).
```{python}

nb = GaussianNB() # create empty model

nb.fit(X_train[continuous], y_train) # fit the model (aka calculate the probabilities)
...
```{python}
# predict
pred_train = nb.predict(X_train[continuous]) # make predictions for data in X_train
pred_train_proba = nb.predict_proba(X_train[continuous])[:,1] # make probabilities for data in X_train
...
# predict
pred_test = nb.predict(X_test[continuous]) # make predictions for data in X_test
pred_test_proba = nb.predict_proba(X_test[continuous])[:,1] # make probabilities for data in X_test
...
for train,test in kf.split(X):
    ...
    # fit model
    nb.fit(X_train[continuous], y_train)
...
#### Build and Fit the Model
Now, we're ready to build the model. First, we create an empty model and put it and our `z` object into a pipeline, and then we fit it. Fitting allows the empty model to use the data we give it (here `X` and `y`) to figure out what the parameters of the model should be. *However*, because we want to do hyperparameter tuning via `GridSearchCV`, we'll need to first create a grid search object, and tell it which hyperparameters to try (these hyperparameter options are stored in the dictionary `msl`).
```{python}

dt = DecisionTreeClassifier() # create empty model
...
# hyperparams
msl = {"decisiontreeclassifier__min_samples_leaf": range(1,50)}

# grid
grid = GridSearchCV(pipe, msl, scoring = "accuracy", cv = 5, refit = True)


grid.fit(X_train, y_train) # fit model (aka find the best parameters)

# what k did it choose??
print("min samples leaf chosen:", grid.best_estimator_.get_params()["decisiontreeclassifier__min_samples_leaf"])
```
...
for train,test in kf.split(X):
    ...
    # hyperparams
    msl = {"decisiontreeclassifier__min_samples_leaf": range(1,31)}
...
Now, we're ready to build the model. First, we create an empty model, and then we fit it. Fitting allows the empty model to use the data we give it (here `X` and `y`) to figure out what the parameters of the model should be. 
```{python}
rf = RandomForestClassifier() # create empty model

rf.fit(X_train, y_train) # fit model (aka find the best parameters)
```
...
```{python}
# predict
pred_train = rf.predict(X_train) # make predictions for data in X_train
pred_train_proba = rf.predict_proba(X_train)[:,1] # make probabilities for data in X_train
...
# predict
pred_test = rf.predict(X_test) # make predictions for data in X_test
pred_test_proba = rf.predict_proba(X_test)[:,1] # make probabilities for data in X_test
...
X = pd.get_dummies(X, columns= ["cp", "slope", "thal"], drop_first = True, dtype = float)
...
#structure of the model
model = kb.Sequential([
    kb.layers.Dense(1, input_shape =[18], activation = "sigmoid"), #input
])
...
```{python}
# predict
pred_train = model.predict(X_train) > 0.5 # make predictions for data in X_train
pred_train_proba = model.predict(X_train)# make probabilities for data in X_train
...
# predict
pred_test = model.predict(X_test) > 0.5 # make predictions for data in X_test
pred_test_proba = model.predict(X_test) # make probabilities for data in X_test
...
## Clustering
In Clustering the general workflow is:


- Create `X` (includes pre-processing, etc)
- Create empty model
- `.fit()`
...
```{python}
burgers = pd.read_csv("https://raw.githubusercontent.com/cmparlettpelleriti/CPSC392ParlettPelleriti/master/Data/burger-king-items.txt", sep = "\t") #file is tsv not csv, so sep = "\t"
...
```{python}
# check null
print(burgers.isnull().sum()) # check if too many are missing
print(burgers.shape) # how many rows, cols to we have?
```
There aren’t many missing values here, compared to the number of rows, so I feel comfortable dropping any row that’s missing a value. So let’s do that.

```{python}
# drop missing
burgers = burgers.dropna() # drop missing rows
burgers = burgers.reset_index() # reset indices
```

Here we drop missing values, and reset the indices. `dropna()` will by default drop any row that’s missing a value. Say we have 5 rows, and our row indices are 0,1,2,3 and 4. If row 2 has a missing value, `dropna()` will drop it. But now our indices are weird, because it goes 0,1,3,4 instead of 0,1,2,3. `reset_index()` will take our data and make sure there are no skipped indices.

Now we can prepare to build our model by subsetting the data to contain only the variables we want to cluster on.

```{python}
features = ["Protein(g)", "Fat(g)", "Sat.Fat(g)"] # specify feature vars

X = burgers[features] # features
```

Then, we'll z-score our inputs. All our inputs should be continuous, since we're using euclidean distance. Because we don't have to worry about a train/test set, we can use `fit_transform()` which does both the fit and the transform step in one.
...
#### Build and Fit the Model
Now, we’re ready to build the model. First, we create an empty model, and then we fit it. Fitting allows the empty model to use the data we give it (here `X`) to figure out what clusters there are.

If you know how many clusters you want ahead of time, you can fit a single model with that number of clusters. However, often you don't know upfront how many clusters you want. In that case, we can try multiple numbers, and see which one does the "best". Typically we'll look at both the average *silhouette score* as well as the *within cluster variance (SSE)*.
...
#### Evaluate the Model
The plots above show us the average silhouette scores and the within cluster variance for each `k`, there's not always *one right* answer for what `k` should be, but here we'll choose 5. So let's refit our clustering model with `k = 5`.

```{python}
km = KMeans(5)
km.fit(X[features])

assignments = km.predict(X[features])
X["assignments"] = assignments
print("Silhouette: ", silhouette_score(X[features], X["assignments"]))
```

We use the average silhouette score to assess how well the model does (although this is *not* the only metric we can use to do so). We also typically plot the clusters, either using the raw variables, or the first two principal components (see PCA section below).

```{python}
(ggplot(X, aes(x = "Fat(g)", y = "Protein(g)", color = "assignments")) + geom_point() + theme_minimal() + 
labs(x = "Fat (in grams)", y = "Protein (in grams)", title = "KMeans Clusters (Fat vs. Protein)"))
```

```{python}
(ggplot(X, aes(x = "Sat.Fat(g)", y = "Protein(g)", color = "assignments")) + geom_point() + theme_minimal() + 
labs(x = "Saturated Fat (in grams)", y = "Protein (in grams)", title = "KMeans Clusters (Saturated Fat vs. Protein)"))
```

```{python}
(ggplot(X, aes(x = "Fat(g)", y = "Sat.Fat(g)", color = "assignments")) + geom_point() + theme_minimal() + 
labs(x = "Fat (in grams)", y = "Saturated Fat (in grams)", title = "KMeans Clusters (Fat vs. Saturated Fat)"))
```
...
# name the PCs
pc_names = ["PC" + str(i) for i in range(len(features))]
...
# plot
(ggplot(X, aes(x = "PC1", y = "PC2", color = "assignments")) + geom_point() + theme_minimal() + 
labs(x = "Principal Component 1", y = "Principal Component 2", title = "KMeans Clusters (First two PCs)"))
```
</details>
...
for k in ks:
  ...
  sil.append(silhouette_score(X[features], gmm.predict(X[features])))
...
#### Evaluate the Model
The plots above show us the average silhouette scores for each `k`, there's not always *one right* answer for what `k` should be, but here we'll choose 4. So let's refit our clustering model with `k = 4`.

```{python}
gmm = GaussianMixture(4)
gmm.fit(X[features])

assignments = gmm.predict(X[features])
X["assignments"] = assignments
print("Silhouette: ", silhouette_score(X[features], X["assignments"]))
```
...
# plot
(ggplot(X, aes(x = "PC1", y = "PC2", color = "assignments")) + geom_point() + theme_minimal() + 
labs(x = "Principal Component 1", y = "Principal Component 2", title = "GMM Clusters (First two PCs)"))
```
...
BUR first: DBSCAN requires two hyperparameters, `eps` (the radius of the neighborhood that we consider for each point) and `min_samples` (the threshold for how many neighbors are required for a point to be a *core* point).

We'll choose 3 for `min_samples`, and use the elbow method to choose `eps`.
...
distances, neighbors = nn.kneighbors(X[features])
...
#### Evaluate the Model
The plot above shows the elbow graph for eps, based on this graph I'm going to choose 0.75 for `eps`.
...
It should be noted that DBSCAN values *density* not *cohesion* so silhoeutte scores can be misleading when assessing DBSCAN.
...
# plot
(ggplot(X, aes(x = "PC1", y = "PC2", color = "assignments")) + geom_point() + theme_minimal() + 
labs(x = "Principal Component 1", y = "Principal Component 2", title = "DBSCAN Clusters (First two PCs)"))
```
</details>
...
We also often look at the dendrogram to visually assess the performance of the model.
